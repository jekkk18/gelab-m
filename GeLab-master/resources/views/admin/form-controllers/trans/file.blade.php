<div class="form-group">
	...
	<input type="hidden" name="{{ $locale }}[{{ $key }}]['file_old']" value="{{$post->translate($locale)->{$key}['file'] }}">
	<input type="hidden" name="{{ $locale }}[{{ $key }}]['name_old']" value="{{$post->translate($locale)->{$key}['name'] }}">
	@endif
    <input type="file" name="{{ $locale }}[file][{{ $key }}]">
