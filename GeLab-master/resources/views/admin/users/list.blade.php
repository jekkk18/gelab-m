                ...
                <tbody>
                    @foreach ($users as $kay => $user)
                        <tr>
                            ...
                            <td><a href="/{{ app()->getLocale() }}/admin/users/destroy/{{ $user->id }}" type="button" class="@if($user->id == auth()->user()->id) disabled @endif btn btn-danger waves-effect width-md waves-light">{{ trans('admin.delete') }}</a></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
