                        ...
                        <tbody>
                            @foreach ($submissions as $key => $submission)
                                <tr {{ $submission->seen == 0 ? 'style=background-color:#d3d3d35e' : ''  }}>
                                    ...
                                    <td>{{ $submission->post->translate(app()->getLocale())->title }}</td>
                                    <td>{{ $submission->name }}</td>
                                    <td>{{ $submission->email }}</td>
                                    <td>{{ substr($submission->text,0, 150) }}</td>
                                    ...
                                    <td><a href="/{{ app()->getLocale() }}/admin/submission/destroy/{{ $submission->id }}" type="button" class=" btn btn-danger waves-effect width-md waves-light">{{ trans('admin.delete') }}</a></td>
